...
**Second-order bottleneck: continuity, captured in H-17.** Precision 0.2049
...
**What is ruled out.**
- Channel capacity at 1/8 (H-06): the lch64 cell at equal FLOP.
- Annotation width mismatch between splits (H-08 in its train/val form):
...
Verdict: **H-05b SUPPORTED (provisional)** - the lane bottleneck is the
absence of 1/4-resolution information. H-06 (channel capacity) is
...
r2_z16 e20 baseline 0.1943 confirms H-05b; 1x-2x is weak; < 1x reverts
H-05b to OPEN, probe A precedent). Standing caveat binds: two prior
...
comparable - the P4B5-STEP3 baseline row is a 20-epoch model and must not be
...
3.62x). This is the continuity half of H-17 showing a pulse; the
...
**P4B-EXP-02 feature provenance (linear probe on the frozen e20 baseline
...
confirmation of H-18 (DA is region-level semantic).
...
**P5-STEP7c seed replication (rule registered in a185e48 before the runs).**
...
not a 4-epoch reading inflated by luck. H-05b stays provisional and nothing
...
**Bottleneck: region-level semantic identification (H-18), not resolution
...
turn pockets, road-like shoulders. P4B-STEP4 (anatomy of false
positives) measures this directly and is the next step before any training.

**What is ruled out.**
- Resolution (H-10): the model has not reached the resolution-defined bar.
- Channel capacity (H-09): same argument.
- Far-field semantics (H-11): the earlier apparent far-field collapse was
  ...
  than R2 in the far band (H-11-alt SUPPORTED).
...
**What is ruled out.**
- Z capacity (H-01): a 4x increase in shared width buys only +0.0235 small
  recall and reduces effective rank.
- Gradient conflict (H-13): probe C rebalanced detection to 20% weight and
  saw -13x noise of mAP50 for +0.27x of DA. Whatever caps DA, it is not
  detection's gradient share.
- Crowding (H-03): not a factor.
- Receptive field (H-03): probe B at 4-epoch showed no effect and 20-epoch
  showed -0.10x; H-03 is closed.
...
2. DA's region-level semantic hypothesis (H-18) is the only DA factor that
   survives; it is not yet measured. P4B-EXP-04 will measure it (size, distance
...
3. Detection has the largest published lever (P2 head) and the smallest
   ...
   separate experiment (P4B-EXP-03), to be registered before running.
4. Lane continuity (H-17) is opened by the precision/recall split; the
...
## P4B-EXP-03. Detection — the binding constraint was on the supervision side
...
1. **H-20 (grid resolution binds) is now a priori less likely.** The sub-cell
...
### P4B-EXP-03 D3: adding a stride-4 detection level makes detection WORSE (H-20 REJECTED, confounded)
...
least this intervention does not find it. H-20 is REJECTED as written.
...
+1618 params), registered as **H-21** and not yet run.
...
detection line therefore has exactly one open cell (dp2b/H-21), and a negative
dp2b closes it.

### P4B-EXP-03 status board

| verdict | content | outcome |
|---|---|---|
| D1 (primary, H-19) | danc mAP50 gain >= 2x noise | **PASS** +0.1543 = 10.57x |
...
| D3 (H-20) | dp2a - danc >= 1x noise | **FAIL** -0.0447 = -3.06x (confounded, see above) |
