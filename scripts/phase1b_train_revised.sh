...
EXP=experiments/phase1b/revised
mkdir -p "$EXP"
log() { echo "$(date +%H:%M:%S) $*" | tee -a "$EXP/status.txt"; }

log "STAGE U (uniform-width heads)"
$PY training/train.py --config configs/phase1b_train_stage_u.yaml --init experiments/phase1b/exp_train_a/checkpoint.pt \
...
$PY training/train.py --config configs/phase1b_train_stage_c_router.yaml --init "$EXP/exp_U/checkpoint.pt" \
...
$PY training/train.py --config configs/phase1b_train_stage_d.yaml --init "$EXP/exp_C/checkpoint.pt" \
