...
for s in 2; do
  log "s$s B"
  $PY training/train.py --config configs/phase1b_train_stage_b.yaml --seed $s --init "$OUT/dynA_s$s/checkpoint.pt" \
      --epochs 2 --outdir "$OUT/dynB_s$s" > "$OUT/dynB_s$s.log" 2>&1 || { log "s$s B FAIL"; exit 1; }
  log "s$s C"
  $PY training/train.py --config configs/phase1b_train_stage_c.yaml --seed $s --init "$OUT/dynB_s$s/checkpoint.pt" \
      --epochs 2 --outdir "$OUT/dynC_s$s" > "$OUT/dynC_s$s.log" 2>&1 || { log "s$s C FAIL"; exit 1; }
  log "s$s D"
  $PY training/train.py --config configs/phase1b_train_stage_d.yaml --seed $s --init "$OUT/dynC_s$s/checkpoint.pt" \
...
log "=== ExpA analysis ==="
$PY scripts/phase2c_expa_analyze.py "$OUT"
log "EXP-A COMPLETE"
