...
OUT=experiments/phase2b/expI_kd_dynamic
mkdir -p "$OUT"
...
$PY training/train.py --config configs/phase1b_train_stage_b_offkd.yaml \
    --init "experiments/phase2b/expF_single_teacher/kd_yolop/checkpoint.pt" --epochs 2 \
    --num-images 10000 --outdir "$OUT/kdD_B" > "$OUT/kdD_B.log" 2>&1 || { log "  kdD_B FAIL"; tail -3 "$OUT/kdD_B.log"; exit 1; }
log "  kdD_B done"
$PY training/train.py --config configs/phase1b_train_stage_c_offkd.yaml \
...
log "  kdD_C done"
$PY training/train.py --config configs/phase1b_train_stage_d_offkd.yaml \
